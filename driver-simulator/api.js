...
function updateVehicleLocation(vehicleId, lat, lng) {
  request(`/vehicles/${vehicleId}/locations`, 'POST', {lat, lng, at: new Date().toISOString()}, res => {
    console.log(`Vehicle ${vehicleId.split('-')[0]} moved to: ${lat}, ${lng}`);
  });
}
...
function request(path, method, body, cb) {
  ...
  req.on('error', error => {
    if (error.code === 'ECONNREFUSED') {
      console.log(`The simulator couldn't reach the API at ${API_HOST}:${API_PORT}`);
    } else {
      console.log(error);
    }
  });
